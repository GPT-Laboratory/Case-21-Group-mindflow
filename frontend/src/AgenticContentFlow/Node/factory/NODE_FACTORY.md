...
The unified node system provides a single, simplified factory framework for all node types. It eliminates the distinction between cell and container frames while maintaining essential functionality through group-based behavior and runtime-determined styling. The `FrameJSON` is completely group-agnostic - the same frame structure works for all node types.

## Key Principles

### **Single Source of Truth**
- One frame type: `FrameJSON`
...
1. **`FrameJSON`** - Single frame type for all nodes (group-agnostic)
...
shared/
├── types/
│   └── FrameJSON.ts          # Single frame type (group-agnostic)
├── components/
...
### **FrameJSON Interface**
See: `src/AgenticContentFlow/Node/factories/shared/types/FrameJSON.ts`
