...
interface ProcessControlsProps {
  ...
  className?: string;
  /** Whether controls should be disabled (e.g., during generation) */
  disabled?: boolean;
}
  ...
  return (
    <div className={`flex items-center gap-1 relative ${className}`}>
        ...
        >
          {waitingForApproval ? (
            ...
            <ControlButton
              ...
              onClick={() => !disabled && onAutoApproveToggle?.()}
              icon={<ApprovalIcon />}
              active={autoApprove}
              disabled={disabled}
            />
          ...
          {showApprovalDropdown && !waitingForApproval && !disabled && (
            <div
      ...
      <ControlButton
        ...
        disabled={(!isProcessing && !isLooping) || disabled}
      />
