...
export const PropertiesTab: React.FC<PropertiesTabProps> = ({ 
  ...
  // Memoize the data separation logic to prevent repeated processing
  const { actualTemplateData, actualInstanceData } = useMemo(() => {
    ...
    if (!formData.hasOwnProperty('templateData') && !formData.hasOwnProperty('instanceData')) {
      // Only migrate once per node to prevent spam
      const migrationKey = `${nodeId}-${nodeType}`;
      if (!migratedNodes.has(migrationKey)) {
        ...
        // Only log migration once per node
        console.log('Migrated old flat structure for node:', nodeId, {
          nodeType,
          instanceFields: Object.keys(actualInstanceData).length,
          templateFields: Object.keys(actualTemplateData).length
        });
        
        migratedNodes.add(migrationKey);
      }
    } else {
      ...
      if (!loggedCombinations.has(logKey)) {
        console.log('Using new structure for node:', nodeId, {
          nodeType,
          instanceFields: Object.keys(actualInstanceData).length,
          templateFields: Object.keys(actualTemplateData).length
        });
    ...
    return { actualTemplateData, actualInstanceData };
  }, [formData, nodeId, nodeType, templateDefaults]);
