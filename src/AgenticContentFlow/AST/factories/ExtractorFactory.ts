import { ASTExtractor, ASTTraverser } from '../interfaces/CoreInterfaces';
...
export class ExtractorFactory {
   ...
   * @throws ASTError if creation fails
   */
  private static createFunctionExtractor(traverser: ASTTraverser): FunctionExtractor {
    try {
   ...
   * @throws ASTError if creation fails
   */
  private static createCallExtractor(traverser: ASTTraverser): CallExtractor {
    try {
   ...
   * @throws ASTError if creation fails
   */
  private static createVariableExtractor(traverser: ASTTraverser): VariableExtractor {
    try {
   ...
   * @throws ASTError if creation fails
   */
  private static createCommentExtractor(traverser: ASTTraverser): CommentExtractor {
    try {
   ...
   * @throws ASTError if creation fails
   */
  private static createDependencyExtractor(traverser: ASTTraverser): DependencyExtractor {
    try {
  ...
  static createExtractorsWithCustomTraverser(
    ...
    try {
      ...
      const traverser = new ASTTraverserClass(maxDepth);
